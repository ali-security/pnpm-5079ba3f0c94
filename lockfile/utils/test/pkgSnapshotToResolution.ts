import { pkgSnapshotToResolution, type TarballResolution } from '@pnpm/lockfile.utils'
...
test('pkgSnapshotToResolution() rejects a remote tarball resolution that has no integrity', () => {
  ...
  // A registry package whose entry has neither integrity nor tarball is
  // equally unverifiable: the tarball URL would be reconstructed from the
  // dependency path and the bytes accepted as-is.
  expect(() => pkgSnapshotToResolution('foo@1.0.0', {
...
test('pkgSnapshotToResolution() allows git-hosted and file: tarballs to lack integrity', () => {
  ...
  // the integrity check, matching the URL fallback used when picking
  // the fetcher.
