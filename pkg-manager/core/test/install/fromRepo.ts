...
test('git-hosted repository is not added to the store if it fails to be built', async () => {
  ...
  // The package has to be allowed to run its build scripts, otherwise the
  // installation would fail before the build is even attempted.
  ...
  await expect(
    addDependenciesToPackage({}, ['pnpm-e2e/prepare-script-fails'], opts())
  ).rejects.toThrow()
