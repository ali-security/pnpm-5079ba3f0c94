...
//   the wrong host. The `certfile`/`keyfile` path variants only ever reach
//   the merged config in their URL-scoped `//host/:certfile=...` and
//   `//host/:keyfile=...` forms, which are already pinned to their
//   authoring registry by construction.
//
...
// `null`/`undefined` values come from npm-conf's defaults layer (it pre-fills
// `cert: null`, `key: null`, `ca: null`, `cafile: null`). Empty strings come
// from the `npm_config_*` environment source, which materializes unset
// settings as `""`. Neither is a real credential, so neither may be rescoped:
// doing so would create meaningless URL-scoped entries and emit a deprecation
// warning on every single config load.
function isDeclared (value: unknown): boolean {
  return value != null && value !== ''
}

// The nerf dart of a registry URL: `//host[:port]/path/`. This is the same
// three-liner that `@pnpm/config.nerf-dart` exports, inlined here because
// that package is not a dependency of `@pnpm/config`.
...
export function rescopeUnscopedCreds (
  ...
  //
  // Unset settings (see `isDeclared`) are not real credentials and are left
  // alone, otherwise we'd create URL-scoped empty entries that downstream
  // consumers would choke on.
  if (!UNSCOPED_RESCOPABLE_KEYS.some(key => Object.hasOwn(source, key) && isDeclared(source[key]))) {
    return source
  }
  ...
  try {
    ...
    const dropped = UNSCOPED_RESCOPABLE_KEYS.filter(key => Object.hasOwn(source, key) && isDeclared(source[key]))
  ...
  for (const key of UNSCOPED_RESCOPABLE_KEYS) {
    if (!Object.hasOwn(source, key) || !isDeclared(source[key])) continue
