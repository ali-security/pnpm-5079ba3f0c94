...
/**
 * Returns true if `dir` is contained in `parentDir`, or if the two are the same directory.
 * Equivalent to the `is-subdir` package, inlined to avoid a new runtime dependency.
 * `path.relative()`, `path.sep`, and `path.isAbsolute()` are platform-aware, so backslash
 * traversal and drive-letter escapes are caught on Windows by the very same code.
 */
